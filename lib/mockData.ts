...
function getRadiusForNode(importance: string, depth: number): number {
  const baseRadius = depth === 0 ? 50 : depth === 1 ? 30 : 15;
  const importanceMultiplier =
    importance === 'critical' ? 1.5 :
    importance === 'high' ? 1.2 :
    importance === 'medium' ? 1 : 0.8;
...
export function generateGraphData(): GraphData {
  const nodes: GraphNode[] = mockMemories.map((memory) => ({
    ...memory,
    radius: getRadiusForNode(memory.importance, getDepthForCategory(memory.category, memory.subcategory, memory.project)),
