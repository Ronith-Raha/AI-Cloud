    ...
    async start(controller) {
      ...
      try {
        ...
        const injectedContext = buildInjectedContext({
          ...
          pinnedSummaries: pinnedNodes.map(
        ...
        const compressionResult = await compressContext(injectedContext);
        const contextToSend = compressionResult.compressed;
        ...
        const [turn] = await db
          ...
          .values({
            ...
            latencyMs,
            providerRequestId,
            compressionEnabled: compressionResult.wasCompressed,
            originalContextTokens: compressionResult.wasCompressed
              ? compressionResult.originalTokens
              : null,
            compressedContextTokens: compressionResult.wasCompressed
              ? compressionResult.compressedTokens
              : null,
            compressionRatio: compressionResult.wasCompressed
              ? compressionResult.compressionRatio
              : null,
            compressionTimeMs: compressionResult.wasCompressed
