...
// Typed graph API layer (preferred names)
export async function getGraph(projectId: string, limit = 50) {
  return getGraphView(projectId, 0, limit);
}
...
export async function editNode(nodeId: string, payload: GraphNodeEditRequest) {
  return editGraphNode(nodeId, payload);
}

export async function deleteNode(nodeId: string) {
  return deleteGraphNode(nodeId);
}

export async function restoreNode(nodeId: string) {
  return restoreGraphNode(nodeId);
}
...
export async function editChatTurn(
  turnId: string,
  payload: ChatTurnEditRequest
) {
  return requestJson<ChatTurnEditResponse>(`/api/chat/turn/${turnId}`, {
    method: "PATCH",
    body: JSON.stringify(payload)
  });
}

export async function getTurns(projectId: string, limit?: number, cursor?: string) {
  const params = new URLSearchParams();
