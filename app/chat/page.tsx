...
export default function ChatPage() {
  ...
  }, [turns, sessions, currentSessionId, pendingSessionId]);
  ...
  useEffect(() => {
    if (!pendingTurnId || turns.length === 0) return;
    const sortedSessions = sessions
      .slice()
      .sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
    const turnIndex = turns.findIndex((turn) => turn.turnId === pendingTurnId);
    if (turnIndex === -1) return;

    for (let i = 0; i < sortedSessions.length; i += 1) {
      const startIndex = turns.findIndex(
        (turn) => turn.turnId === sortedSessions[i].startTurnId
      );
      ...
      const endIndex = nextSession
        ? turns.findIndex((turn) => turn.turnId === nextSession.startTurnId)
        : turns.length;
      if (turnIndex >= startIndex && turnIndex < (endIndex === -1 ? turns.length : endIndex)) {
        setCurrentSessionId(sortedSessions[i].id);
        setPendingTurnId(null);
        setChatSessionKey((prev) => prev + 1);
        break;
      }
    }
  }, [pendingTurnId, turns, sessions]);
  ...
  return (
    <MainLayout>
      <div className="h-[calc(100vh-64px)] flex">
        ...
        <div className="w-80 flex-shrink-0 border-r border-white/10 flex flex-col">
          ...
          <div className="p-6 border-b border-white/10">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-r from-cyan-500 to-purple-500 rounded-xl text-white font-medium hover:opacity-90 transition-opacity active:scale-[0.99]"
            >
              ...
              <div className="flex flex-col items-end gap-2">
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 text-xs rounded-lg bg-white/5 text-white/70 hover:bg-white/10"
                  >
                    Memory panel
                  </button>
                </div>
        ...
        {/* Sliding right panel */}
        <div
          className={`fixed top-16 right-0 h-[calc(100vh-64px)] w-72 border-l border-white/10 bg-black/90 backdrop-blur-xl transition-transform duration-200 ${
