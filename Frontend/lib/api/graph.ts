...
import type { GraphData, GraphLink, GraphNode, ZoomLevel } from "@/types/nexus";
...
function mapNode(node: VizNode): GraphNode {
  ...
  const category = node.pinned
    ? PINNED_CATEGORY
    : derivedCategory || DEFAULT_CATEGORY;
...
function buildSimilarityEdges(nodes: GraphNode[], threshold = 0.25) {
...
export function getRelationshipThreshold(zoomLevel: ZoomLevel): number {
  switch (zoomLevel) {
    case 1: return 0.4;   // Zoomed out: strong relationships only
    case 2: return 0.25;  // Mid: moderate filtering
    case 3: return 0.15;  // Zoomed in: show most
  }
}
...
export function mapGraphViewToGraphData(view: GraphViewResponse): GraphData {
  ...
  const groupNodes = buildGroupNodes(baseNodes);
  const groupLinks: GraphLink[] = [];

  baseNodes.forEach((node) => {
    const groupId = `group:${node.category}`;
    groupLinks.push({
      source: groupId,
      target: node.id,
      strength: 0.5,
  ...
  return {
    nodes: [...groupNodes, ...baseNodes],
    links: [...view.edges.map(mapEdge), ...groupLinks, ...similarityEdges],
    clusters: []
  };
}
